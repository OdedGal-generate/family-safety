...
router.post("/", requireAuth, validate(schemas.createGroup), (req, res) => {
...
router.post("/join", requireAuth, validate(schemas.joinGroup), (req, res) => {
...
router.patch("/:id/requests/:reqId", requireAuth, validate(schemas.reviewRequest), (req, res) => {
  const groupId = Number(req.params.id);
  const reqId = Number(req.params.reqId);
  if (!Number.isInteger(groupId) || !Number.isInteger(reqId) || groupId <= 0 || reqId <= 0) {
